...
#############################################################
    ...
    for _ in pbar:  
        ...
        loss_val, accuracy = evaluate(model, criterion, tstloader, device)
...
##########################################################################
    ...
    torch.save('output_model/'+config["model_type"] + "_full_data_train.pth")
    
    accuracyV(history['accuracies'], config['name']+'val_train', config['model_type'])
    trnV_loss(history['train_losses'],history['val_losses'], config['name']+'val_train', config['model_type'])
